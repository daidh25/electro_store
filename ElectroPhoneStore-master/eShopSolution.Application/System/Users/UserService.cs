...
namespace eShopSolution.Application.System.Users
{
    public class UserService : IUserService
    {
        ...
        public async Task<ApiResult<bool>> IncreaseDailyVisitorCount()
        {
            try
            {
                var currentDate = DateTime.Now.Date;
                var dailyVisitor = await _context.DailyVisitorCounts.FirstOrDefaultAsync(x => x.Date == currentDate);
                if (dailyVisitor == null)
                {
                    dailyVisitor = new DailyVisitorCount { Date = currentDate, Count = 1 };
                    _context.DailyVisitorCounts.Add(dailyVisitor);
                }
                else
                {
                    dailyVisitor.Count++;
